%% =========================================================
%  MISO RIS-Enabled Downlink ISAC-OMA System  (v3-EASY, TDMA)
%  ---------------------------------------------------------
%  OMA-TDMA counterpart to miso_isac_noma_v3_easy.m.
%  Identical channel model, distances, path-loss, power, and noise.
%  The ONLY differences vs the NOMA file are in the multiple-access scheme:
%
%  OMA  : three orthogonal time slots with fractions (t_n, t_f, t_T),
%          per-user MRT (w_n = h_n/||h_n||, w_f = h_f/||h_f||),
%          unconstrained sensing BF (dom. eigvec of G_T'*G_T),
%          full P_tot in every slot, zero inter-user interference.
%
%  NOMA : one shared time slot, cluster MRT (w_c = h_f/||h_f||),
%          power-domain multiplexing, SIC at near user.
%
%  RATES (OMA):
%    R_n = t_n * log2(1 + g_nn * P_tot / sigma2)   g_nn = ||h_n||^2
%    R_f = t_f * log2(1 + g_ff * P_tot / sigma2)   g_ff = ||h_f||^2
%    R_s = t_T * log2(1 + g_sT * P_tot / sigma2)   g_sT = ||G_T w_T||^2
%
%  Baseline time split: t_n = t_f = t_T = 1/3 (the DL policy later learns t).
%
%  Output : ISAC_RIS_OMA_channels_v3_easy.mat
% =========================================================
...
%% --- OMA time fractions (baseline equal split) ---
t_n = 1/3;
t_f = 1/3;
t_T = 1/3;
...
%% --- Outage / QoS thresholds (same as NOMA) ---
gamma_c_th   = 0.3;
gamma_s_th   = 0.1;
...
% OMA achieves much higher rates than NOMA (no inter-user interference,
% full P_tot per slot), so the NOMA thresholds (2.0 / 0.3) are almost never
% violated. To keep BOTH QoS constraints active (~25% baseline violation,
% giving the DL/MAML policy a real feasibility surface to optimise), the
% thresholds are scaled to the OMA rate distribution:
%   R_comm_sum ~ 11.6 bits/s/Hz  ->  R_th_c = 11.0  (comm bites ~13%)
%   R_s        ~ 1.60 bits/s/Hz  ->  R_th_s = 1.0   (sensing bites ~14%)
%   combined Pr(any QoS violated) ~ 25%.
R_th_c = 11.0;
R_th_s = 1.0;

%% --- Print header ---
fprintf('========================================================\n');
fprintf(' MISO RIS-ISAC-OMA v3-EASY  (M=%d, N=%d, samples=%d)\n', M, N, num_samples);
fprintf('========================================================\n');
...
for s = 1:num_samples
    ...
    % 5. OMA beamforming: per-user MRT + unconstrained sensing BF
    w_n = h_n / max(norm(h_n), 1e-15);
    w_f = h_f / max(norm(h_f), 1e-15);

    % Dedicated sensing slot -> no comm interference -> no null-space needed
    A_s = G_T' * G_T;
    A_s = (A_s + A_s') / 2;
    [V_A, D_A] = eig(A_s);
    ...
    % 6. Effective gains (orthogonal slots -> no cross terms)
    g_nn = abs(h_n' * w_n)^2;          % = ||h_n||^2
    g_ff = abs(h_f' * w_f)^2;          % = ||h_f||^2
    g_sT = abs(u_T' * G_T * w_T)^2;    % = ||G_T w_T||^2

    % 7. SINRs (no inter-user interference, full power per slot)
    snr_n = g_nn * P_tot / sigma2;
    snr_f = g_ff * P_tot / sigma2;
    snr_s = g_sT * P_tot / sigma2;

    % 8. Outage checks (time-weighted rates)
...
figure('Name','MISO RIS-ISAC-OMA v3-EASY','NumberTitle','off', ...
...
sgtitle('MISO RIS-ISAC-OMA v3-EASY (equal-time baseline, per-user MRT)','FontSize',13);

plot_file = fullfile(pwd, 'MISO_ISAC_OMA_v3_easy_Results.png');
try
...
%% ===================== SAVE DATASET =====================
output_mat = fullfile(pwd, 'ISAC_RIS_OMA_channels_v3_easy.mat');
save(output_mat, ...
