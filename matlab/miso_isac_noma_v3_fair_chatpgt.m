%% =========================================================
%  MISO RIS-Enabled Downlink ISAC-NOMA System (v3-FAIR)
%  ---------------------------------------------------------
%  Fairer NOMA baseline for RIS-ISAC learning.
%  Keeps random RIS + fixed initialization so NN still learns:
%    - RIS phases
%    - Power allocation
%
%  Changes vs v3-easy:
%   1) M = 4 antennas (more DoF)
%   2) Better channel disparity (3m near vs 25m far)
%   3) Better initial NOMA power split (a_f=0.55, a_n=0.20, a_T=0.25)
%   4) Weighted common beamforming (0.7*h_f + 0.3*h_n)
%   5) Soft sensing nulling (lambda_null penalty instead of hard projection)
%
%  Produces the SAME full output set as miso_isac_noma_v3_easy.m:
%    - per-sample channels / beamformers / SINR / rate storage
%    - decode-chain (SIC) outage + service-rate outage
%    - QoS violation arrays (comm / sensing / any)
%    - header + aggregate console report
%    - 6-panel SINR/rate histogram PNG
%    - ISAC_RIS_NOMA_channels_v3_fair.mat dataset
% =========================================================
...
%% --- Distances (NOMA friendly disparity) ---
d_BR=8;
d_RDn=3;
d_RDf=25;
d_RT=6;
...
%% --- Fair-BF tuning knobs ---
alpha_far   = 0.7;     % weight on far user in common beamformer
alpha_near  = 0.3;     % weight on near user in common beamformer
lambda_null = 0.5;     % soft sensing-nulling penalty
...
% This fair config (M=4, 3m near user, weighted BF) gives very high rates:
%   R_comm_sum ~ 22.5 bits/s/Hz, R_s ~ 4.0 bits/s/Hz, so the easy-file
%   thresholds (2.0 / 0.3) are NEVER violated (~0%). Thresholds are scaled
%   to the fair rate distribution so BOTH QoS constraints stay active
%   (~20% each, ~36% combined) -- a real feasibility surface for the policy:
%     R_th_c = 21.7  (comm bites ~20%)
%     R_th_s = 2.9   (sensing bites ~20%)
R_th_c = 21.7;
R_th_s = 2.9;

%% --- Sanity print ---
fprintf('========================================================\n');
fprintf(' MISO RIS-ISAC-NOMA v3-FAIR  (M=%d, N=%d, samples=%d)\n', M, N, num_samples);
fprintf('========================================================\n');
...
fprintf('Fair-BF: common w_c = %.2f*h_f + %.2f*h_n   soft-null lambda=%.2f\n\n', ...
        alpha_far, alpha_near, lambda_null);
...
for s=1:num_samples
    ...
    %% Random RIS (NN will optimize later)
    ...
    %% ---------- FAIRER NOMA BF ----------
    w_c = alpha_far*h_f + alpha_near*h_n;
    w_c = w_c/max(norm(w_c),1e-15);

    w_n = w_c;
    w_f = w_c;

    %% Soft sensing nulling
    H_u = [h_n, h_f];
    ...
    gsn = abs(u_T'*G_T*w_n)^2;
    gsf = abs(u_T'*G_T*w_f)^2;
    ...
    snr_n = (gnn*a_n*P_tot) / ...
        (delta_f*gnf*a_f*P_tot + gnT*a_T*P_tot + sigma2);

    snr_s = (gsT*a_T*P_tot) / ...
        (delta_f*gsf*a_f*P_tot + ...
         delta_n*gsn*a_n*P_tot + sigma2);

    %% SIC cross-SINRs (near decodes far; sensing decodes comm)
    snr_n_to_f  = (gnf*a_f*P_tot) / ...
                  (gnn*a_n*P_tot + gnT*a_T*P_tot + sigma2);
    snr_BS_to_f = (gsf*a_f*P_tot) / ...
                  (gsn*a_n*P_tot + gsT*a_T*P_tot + sigma2);
    snr_BS_to_n = (gsn*a_n*P_tot) / ...
                  (delta_f*gsf*a_f*P_tot + gsT*a_T*P_tot + sigma2);

    %% Decode-chain outage events
    if snr_f < gamma_c_th
    ...
    if ~(snr_BS_to_f > gamma_SIC_th && snr_BS_to_n > gamma_SIC_th && snr_s > gamma_s_th)
...
figure('Name','MISO RIS-ISAC-NOMA v3-FAIR','NumberTitle','off', ...
...
sgtitle('MISO RIS-ISAC-NOMA v3-FAIR (M=4, weighted BF + soft null, MRT baseline)','FontSize',13);

plot_file = fullfile(pwd, 'MISO_ISAC_NOMA_v3_fair_Results.png');
try
...
%% ===================== SAVE DATASET =====================
output_mat = fullfile(pwd, 'ISAC_RIS_NOMA_channels_v3_fair.mat');
save(output_mat, ...
